...
def get_int_setting(param, default):
    try:
        ...
        print("Error while parsing {f}, check restapi.conf : {s}".format(f=param, s=str(e)))
...
def get_from_cache(f, overrideName=None, *args, **kwargs):
    ...
    try:
        ...
        if data is not None:
            print("get {cache_key} from cache")
        ...
        if isinstance(resp, dict):
            print("set {cache} in cache with expiration {expiration}")

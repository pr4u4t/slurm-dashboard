...
if auth_enabled:
    ...
    if settings.has_option('config', 'secret_key'):
        ...
        secret_key_file = '../conf/secret.key'

    # check secret key file exist or print error otherwise
    if not os.path.exists(secret_key_file):
        print("Secret key file {s} does not exists".format(s=secret_key_file))

    try:
        secret_key = open(secret_key_file, 'rb').read()
    except IOError:
        print("IO error with secret key file {s}".format(s=secret_key_file))
        # fallback to bad secret key
        secret_key = b"badsecretkey"

    secret_key += bytes(platform.node(),'ascii')
    ...
    if settings.has_option('roles', 'guests'):
        ...
        if guests_allowed_s == 'enabled':
            ...
            print("invalid value '{s}' for guests parameter, guests will be "
                  "disabled.".format(s=guests_allowed_s))
    ...
    if settings.has_option('roles', 'trusted_sources'):
        ...
        if trusted_sources_allowed_s == 'enabled':
            ...
            print("invalid value '{s}' for trusted_sources parameter, trusted"
                  " sources will be disabled.".format(s=trusted_sources_allowed_s))
    ...
    if settings.has_option('roles', 'all'):
        ...
        if all_s == 'disabled':
            ...
            print("invalid value '{all_s}' for all parameters, all role will be "
                  "enabled.")
...
class User(object):
    ...
    def get_groups_from_ldap(login, password):
        ...
        try:
            ...
            if settings.has_option('ldap','bind_dn'):
                bind_dn = settings.get('ldap', 'bind_dn')
            if settings.has_option('ldap','bind_pw'):
                bind_pw = settings.get('ldap', 'bind_pw')
            

            # authenticate user on ldap
            if bind_dn != None and bind_pw != None:
                conn.bind(bind_dn,bind_pw)

            query = "(&(objectClass=posixAccount)(uid=%s))" % (login)
            result = conn.search_s(base_people, ldap.SCOPE_SUBTREE, query,["dn"])

            if len(result) != 1:
                print("User search failed for user: %s" % (login))
                raise AuthenticationError

            #user_dn = "uid=%s,%s" % (login, base_people)
            conn.simple_bind(result[0][0], password)

            print("User %s authenticated" % (login))

            # search for user's groups
            look_filter = "(|(&(objectClass=*)(member=%s)))" % (result[0][0])
            ...
            print("No result found: %s ".format(str(e)))
            raise AuthenticationError
    ...
    def generate_auth_token(
        ...
        print("generate_auth_token : token -> {token}")
        return token
    ...
    def verify_auth_token(token):
        ...
        try:
            data = s.loads(token)
            print("verify_auth_token : data -> login id: {id} role: {role}".format(id=data['login'], role=data['role']))
